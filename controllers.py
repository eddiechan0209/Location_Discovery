...
def add_contact():
    ...
    id = db.contact.insert(
        ...
        more_content=request.json.get('more_content'),
        name=name,
        email=email
    )
...
def get_likes():
    ...
    post_id = request.params.get('post_id')
    rows = db((db.thumbs.contact == post_id) &
             (db.thumbs.rating == 1)).select()
    names = []
    for row in rows:
        names.append(row.rater_name)
    return dict(names=names)

@action('get_dislikes')
@action.uses(url_signer.verify(), db, auth.user)
def get_likes():
    """Returns the dislikes for a post."""
    post_id = request.params.get('post_id')
    rows = db((db.thumbs.contact == post_id) &
             (db.thumbs.rating == -1)).select()
    names = []
